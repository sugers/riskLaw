...
<script>
    ...
    export default {
        props: ['currentAreaId', 'currenrAreaText'],
        data() {
            return {
                dataObj: {
                    companyName: '',
        ...
        watch: {
            currentAreaId(newVal) {
                this.currentid = newVal;
            },
            currenrAreaText(newVal) {
                let currentData = JSON.parse(newVal);
                this.currentid=currentData.ID;
                this.dataObj.staturadio=currentData.status
